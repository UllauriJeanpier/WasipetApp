...
import {getBreedbyid} from '../APIRequest/breedRequest';
...
export default class Promociones extends Component {
  ...
  async UNSAFE_componentWillMount() {
    ...
    this.setState({
      ...
      pet: await getPetbyOwnew(token),
    });

    this.setState({
      breed: await getBreedbyid(this.state.pet.breed),
    });

    this.setState({
      petType: this.state.breed.type,
    });
  }
  ...
  _renderItem = ({item, index}) => {
    return (
      <TouchableOpacity
        onPress={() =>
          this.props.navigation.navigate('ProductosCanjeablesPromocion', {
            brand: item,
