...
import {
  ...
  updatePetById,
  addAditionalPet,
} from '../APIRequest/petRequest';
import DateTimePicker from '@react-native-community/datetimepicker';
import ImagePicker from 'react-native-image-picker';
import AsyncStorage from '@react-native-community/async-storage';
import {connect} from 'react-redux';
import {refreshPet, refreshBreeds} from '../redux/actions/userActions';
...
import {PET_GENDERS, PET_TYPES} from '../utils/constantes';
...
class AjustePerfilMascota extends Component {
  ...
  async UNSAFE_componentWillMount() {
    ...
    try {
      ...
      if (this.state.state === 'edit') {
        ...
        if (breed) {
          this.props.refreshBreeds(breed.type);
          await this.setState({
            petId: pet._id,
            breed: breed.name,
            name: pet.name,
            photo: pet.photo,
            gender: pet.gender !== 'null' ? pet.gender : null,
            BornDate: pet.BornDate !== 'null' ? pet.BornDate : null,
            allergies: pet.allergies !== 'null' ? pet.allergies : null,
            disability: pet.disability !== 'null' ? pet.disability : null,
            description: pet.description !== 'null' ? pet.description : null,
            petType: breed.type,
            initialfetch: false,
          });
        } else {
  ...
  async handleSubmit() {
    ...
    //console.log(user);
    if (name !== '' && photo && gender && breed && petType && token) {
      const pet = {
        id: petId,
        user,
        token,
      ...
      try {
        let response =
          state === 'edit'
            ? await updatePetById(pet)
            : await addAditionalPet(pet, user);
        if (response.name !== null) {
          ...
          Alert.alert(
            'Error',
            'Verifique su conexion a internet o intetelo mas tarde',
            [{text: 'Cerrar'}],
            {cancelable: true},
          );
        }
      ...
      Alert.alert(
        ...
        'Debe agregar una foto y llenar los 4 primeros campos para registrar una mascota',
        [{text: 'Cerrar'}],
        {cancelable: true},
      );
      this.setState({
          ...
          {!this.state.initialfetch ? (
            <View style={styles.container}>
              <View style={styles.subContainer}>
                ...
                <View style={styles.labelContainer}>
                  <Text style={styles.labeltext}>¿Qué mascota tienes?</Text>
                </View>

                <RNPickerSelect
                  placeholder={{
                    label: this.state.petType
                      ? this.state.petType === 'Dog'
                        ? 'Perro'
                        : 'Gato'
                      : 'Elije que mascota tienes',
                    value: null,
                  }}
                  ...
                  onValueChange={async value => {
                    ...
                    this.props.refreshBreeds(value);
                  }}
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    refreshBreeds: type => {
      return dispatch(refreshBreeds(type));
    },
  };
};
